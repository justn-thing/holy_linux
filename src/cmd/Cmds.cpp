...
namespace cmds {
    ...
    int rmdir_(const CmdParams& param) {
        ...
        if (!target) {
            ...
            return 1;
        } if (target == FS::current || (recursive && IsAncestor(target, FS::current))) {
            alert(msg::cant_remove_self, stx::yellow);
            return 1;
        }
        if (ContainsLockedNode(target) && NotSudo(param)) return 1;
        ...
        if (!RemoveNode(target, recursive))
            return 1;
